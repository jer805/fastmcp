...
Consumed `jti` values are recorded in `client_storage`, alongside the rest of the proxy's state. A deployment that already points `client_storage` at a shared backend gets replay protection across every replica with no additional configuration:

```python
from key_value.aio.stores.redis import RedisStore

storage = RedisStore(url="redis://localhost:6379/0")

proxy = OAuthProxy(
    ...,
    client_storage=storage,
    identity_assertion=IdentityAssertion(trusted_issuers=[ISSUER]),
)
```

To keep high-churn replay state off the store holding client registrations and tokens, point it somewhere else with `identity_assertion_jti_store`.

<Warning>
The replay check is a read followed by a write, not an atomic set-if-absent — the underlying key-value interface exposes no such primitive — so two requests carrying the same assertion that arrive genuinely concurrently can both be admitted. The window is narrow, but it is not zero.

Leaving `client_storage` at its default means replay state is per-process, and a deployment running multiple workers or replicas will accept the same assertion once per process. Revocation of ID-JAG access tokens is per-process regardless: those tokens are self-contained, so revocation is tracked in memory until the token's (short, 5-minute default) natural expiry. For strict revocation enforcement across replicas, configure sticky routing so a given client's requests reach the same process. CIMD `private_key_jwt` replay protection remains per-process as well.
</Warning>
