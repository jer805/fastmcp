...
<Warning>
By default, replay protection is per-process: each server process tracks seen `jti` values in an in-memory store, so a horizontally-scaled deployment running multiple workers or replicas could accept the same assertion once per process. Pass a shared store — Redis, Postgres, or any other `AsyncKeyValue`-compatible backend — via `identity_assertion_jti_store` to share replay state across replicas:
...
proxy = OAuthProxy(
    ...,
    identity_assertion=IdentityAssertion(trusted_issuers=[...]),
    identity_assertion_jti_store=RedisStore(url="redis://localhost:6379/0"),
)
```

This closes the cross-replica gap for practical purposes, though the underlying check is a TTL-checked read followed by a write rather than an atomic set-if-absent, so a backend that itself enforces key uniqueness (e.g. Redis `SET NX`) gives the strongest guarantee. Revocation of ID-JAG access tokens is a separate, still per-process mechanism: they are self-contained, so revocation is tracked in-process until the token's (short, 5-minute default) natural expiry. For deployments that require strict revocation enforcement across replicas, configure sticky routing so a given client's requests reach the same process. This mirrors the posture of CIMD `private_key_jwt` replay protection, which is also per-process.
</Warning>
